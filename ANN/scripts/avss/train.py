...
def main():
    ...
    # audio_backbone = audio_extractor(cfg, 'cuda' if torch.cuda.is_available() else 'cpu')
    # audio_backbone.cuda()
    # audio_backbone.eval()
    ...
    for epoch in range(cfg.process.train_epochs):
        ...
        for n_iter, batch_data in enumerate(train_dataloader):
            ...
            output, mask_feature, mi_map = model(audio, imgs, vid_temporal_mask_flag)
            loss, loss_dict = IouSemanticAwareLoss(
                output, mask_feature, mi_map, label, gt_temporal_mask_flag, **cfg.loss)
        ...
        if epoch >= cfg.process.start_eval_epoch and epoch % cfg.process.eval_interval == 0:
            ...
            with torch.no_grad():
                for n_iter, batch_data in enumerate(val_dataloader):
                    ...
                    output, _ , _ = model(audio, imgs, vid_temporal_mask_flag)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home/songyu/vit-llm/yzy/MI_AVS/config/avss/AVSegFormer_pvt2_avss.py', help='config file path')
    parser.add_argument('--log_dir', type=str,
                        default='/home/songyu/vit-llm/yzy/MI_AVS/work_dir_avss', help='log dir')
    parser.add_argument('--checkpoint_dir', type=str,
                        default='/home/songyu/vit-llm/yzy/MI_AVS/work_dir_avss', help='dir to save checkpoints')
    parser.add_argument("--session_name", default="avss",
                        type=str, help="the AVSS setting")
