...
class AVSTransformerEncoderLayer(nn.Module):
    def __init__(self, dim=256, ffn_dim=2048, dropout=0.1, num_levels=3, num_heads=8, num_points=4):
        ...
        self.pre_activation_norm = nn.LayerNorm(ffn_dim)  # 在MultiSpike前添加LayerNorm
        self.activation = MultiSpike()
    ...
    def ffn(self, src):
        linear1_out = self.linear1(src)
        normalized = self.pre_activation_norm(linear1_out)  # 在MultiSpike前应用归一化
        activated = self.activation(normalized)
        src2 = self.linear2(self.dropout2(activated))
...
class AVSTransformerDecoderLayer(nn.Module):
    ...
    def ffn(self, src):
        ...
        normalized = self.pre_activation_norm(linear1_out)  # 在MultiSpike前应用归一化
        activated = self.activation(normalized)
        src2 = self.linear2(self.dropout3(activated))
