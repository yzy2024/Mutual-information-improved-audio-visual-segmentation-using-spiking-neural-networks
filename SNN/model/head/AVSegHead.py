...
#         # predict output mask
#         # mask_feature = torch.ones((5, 256, 128, 128)).to(device=outputs[-1].device)  # 保持原始形状 (b, c, h, w)，值全为 1
#         # pred_feature = torch.einsum('bqc,bchw->bqhw', outputs[-1], mask_feature)
...
#         # pred_mask = pred_feature
#         # pred_mask = self.fc(pred_mask)
#         cur_fpn = self.lateral_conv(feat14)  # [bs, embed_dim, H, W]
#         pred_feature = torch.zeros(bs, self.query_num, cur_fpn.size(2), cur_fpn.size(3), device=cur_fpn.device)  # [bs, query_num, H, W]
#         pred_feature = self.mlp(pred_feature)  # [bs, embed_dim, H, W]
#         pred_mask = self.fc(pred_feature)
...
from MultiSpike import MultiSpike
from model.utils import build_transformer, build_positional_encoding, build_fusion_block, build_generator
from ops.modules import MSDeformAttn
from torch.nn.init import normal_
from torch.nn.functional import interpolate
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        for i in range(num_layers - 1):
            self.norms.append(nn.GroupNorm(min(32, h[i]), h[i]))
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            if i < self.num_layers - 1:
                x = layer(x)
                x = self.norms[i](x)  # 在MultiSpike前添加归一化
                x = self.spike(x)
            else:
                x = layer(x)
        return x
...
class AVSegHead(nn.Module):
    def __init__(self,
        ...
        self.out_conv = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim,
                      kernel_size=3, stride=1, padding=1),
            nn.GroupNorm(32, embed_dim),
            #nn.GroupNorm(32, embed_dim),  # 在MultiSpike前添加归一化
            MultiSpike()
        )
        ...
        self.attn_fc = nn.Sequential(
            ...
            Interpolate(scale_factor=scale_factor, mode="bilinear"),
            nn.Conv2d(128, 32, kernel_size=3, stride=1, padding=1),
            nn.GroupNorm(min(32, 32), 32),  # 在MultiSpike前添加归一化
            MultiSpike(),
            nn.Conv2d(32, num_classes, kernel_size=1,
                      stride=1, padding=0, bias=False)
        )
    ...
    def forward(self, feats, audio_feat):
        ...
        mask_feature = self.out_conv(mask_feature)
        if hasattr(self, 'fusion_block'):
            mask_feature, mi_map = self.fusion_block(mask_feature, audio_feat)
        ...
        return pred_mask, mask_feature, mi_map
